...
import { MessageSquare, ArrowRight, Clock, CheckCircle2, XCircle, ArrowLeft } from 'lucide-react';
import { Button } from '../../components/ui/button';
import { NegotiationThread } from '../../../lib/types';

export const MyNegotiations: React.FC = () => {
  ...
  // Filter negotiations initiated by this kid
  const kidThreads = negotiations.filter((t) => t.initiatedByProfileId === currentProfile.id);

  // Find currently selected thread object
  const selectedThread = kidThreads.find((t) => t.id === selectedThreadId);

  // Sorting: open first, then by updatedAt descending
  const sortedThreads = [...kidThreads].sort((a, b) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-foreground">My Negotiations</h2>
        <p className="text-sm text-muted-foreground">Discuss and adjust token values of activities and costs of rewards</p>
      </div>

      {sortedThreads.length === 0 ? (
        <EmptyState
          icon={MessageSquare}
          title="No negotiations started"
          description="Click 'Negotiate Cost' on a reward or 'Negotiate Value' on a task card to start a discussion!"
        />
      ) : (
        <div className="grid gap-6 md:grid-cols-3 items-start">
          ...
          <div className={`md:col-span-1 space-y-3 ${selectedThreadId ? 'hidden md:block' : 'block'}`}>
            <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground px-1 mb-2">
              Discussion Threads ({sortedThreads.length})
            </h3>
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center justify-between text-xs text-muted-foreground">
                        <span>Offer: <span className="font-bold text-foreground">{thread.currentOfferValue}</span></span>
                      ...
                      <div className="flex items-center justify-between text-[10px] text-muted-foreground/80 font-medium">
                        <span>{thread.targetType === 'activity' ? 'Activity' : 'Reward'}</span>
                        <span>{formatDate(thread.updatedAt)}</span>
                      </div>

                      {isActionNeeded && (
                        <div className="text-[10px] font-bold text-primary mt-1">
                          Parent offered counter! Needs action
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <div className={`md:col-span-2 ${!selectedThreadId ? 'hidden md:flex md:items-center md:justify-center md:h-[400px] md:border md:border-dashed md:border-border md:rounded-xl md:bg-muted/10' : 'block'}`}>
            {selectedThread ? (
              ...
              <div className="text-center text-sm text-muted-foreground italic py-12">
                Select a negotiation thread from the list to view the offer history.
              </div>
            )}
          </div>
        </div>
