...
// localStorage-mode only: live families are created at parent signup (Phase 4).
export async function ensureFamily(): Promise<string> {
  if (!useLocalStorage) {
    throw new Error('ensureFamily is localStorage-mode only; live families are created at signup');
  }

  let familyId = localStorage.getItem('fam_id');

  const needsLocalSeed = !familyId || !localStorage.getItem(`profiles_${familyId}`);
